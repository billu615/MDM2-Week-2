...
# 训练数据路径
flags.DEFINE_string('train_data_path', 'data/cifar-10-batches-bin/data_batch*', 'Filepattern for training data.')
# 测试数据路径
flags.DEFINE_string('eval_data_path', 'data/cifar-10-batches-bin/test_batch.bin', 'Filepattern for eval data')
# 图片尺寸
flags.DEFINE_integer('image_size', 32, 'Image side length.')
# 训练过程数据的存放路径
flags.DEFINE_string('train_dir', 'temp/train', 'Directory to keep training outputs.')
# 测试过程数据的存放路径
flags.DEFINE_string('eval_dir', 'temp/eval', 'Directory to keep eval outputs.')
# 测试数据的Batch数量
...
# 模型存储路径
flags.DEFINE_string('log_root', 'temp', 'Directory to keep the checkpoints. Should be a parent directory of FLAGS.train_dir/eval_dir.')
# GPU设备数量（0代表CPU）
...
def train(hps):
    ...
    # 学习率更新器，基于全局Step
    class _LearningRateSetterHook(tf.compat.v1.train.SessionRunHook):
